...
class BigramLanguageModel(nn.Module):
    ...
    def seq2seq(self, idx):
        # forward of seq2seq of gpt-2
        logits, loss = self(idx)
        logits = logits.view(-1, vocab_size)
        probs = F.softmax(logits, dim=-1)
        #  probs [B,blocksize,vocab_size]
        out = torch.max(logits, dim=1, keepdim=True)[1]
        #out = torch.multinomial(probs, num_samples=1) 
        return out

model = torch.load("finetuned_genegpt.pt")
m = model.to(device)
...
for i, j in tqdm(valid[["x", "y"]].values):
    input_ids = torch.tensor(encode(i), dtype=torch.long).to(device)
    label = j.split("#")[0]
    test_input = torch.stack([input_ids])
    all_output = decode(m.seq2seq(test_input).view(128).tolist())
    output_f = all_output.split('#')[0]
    result.append({"input": i.split('#')[0], "label": label, "prediction": output_f})
    if output_f == label:
        right += 1
df = pd.DataFrame(result)
df.to_csv("tcr_finetune_prediction.csv")
